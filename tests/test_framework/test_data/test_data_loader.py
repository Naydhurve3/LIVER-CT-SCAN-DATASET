...
from src.framework.data.lits_dataset import (
    DataPathManager, VolumeWiseSplitter,
...
class TestDatasetConfig(unittest.TestCase):
    ...
    def test_output_dirs_exist(self):
        ...
        self.assertTrue(Path("data/splits").exists())


class TestDataPathManager(unittest.TestCase):
    def setUp(self):
        self.manager = DataPathManager(Path("."), Path("."))
...
class TestLiverTumor2DDataset(unittest.TestCase):
    ...
    def test_dataset_aligns_native_mask_shape_without_transform(self):
        small_mask = Image.fromarray(np.ones((32, 32), dtype=np.uint8) * 255)
        small_mask.save(self.mask_dir / "mask-000-000.png")
        dataset = LiverTumor2DDataset([0], self.volume_index)
        item = dataset[0]
        self.assertEqual(item['image'].shape, item['mask'].shape)
        self.assertEqual(item['mask'].shape, (1, 64, 64))
